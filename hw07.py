...
class Card:
    def __init__(self, name):
        bag = [x for x in range(1, 91)]  # Генерация мешка с боченками
        self.card = [
            __class__.generation(bag),
            __class__.generation(bag),
            __class__.generation(bag)
        ]
        self.name = name
        self.count_barrel = 15  # Счетчик оставшихся номеров на карточке

    @staticmethod
    def generation(bag):  # Функция генерации строки карточки
        string = ['' for _ in range(9)]
        for x in range(8, 3, -1):
            digit = random.randint(0, x)  # Номер элемента строки, который заполняется
            while string[digit] != '':
                digit += 1
...
bag = [x for x in range(1, 91)]  # Мешок с боченками
while True:
    if len(bag) < 1:
    ...
    print(computer)
    print(player)
    user_choice = input('Зачеркнуть цифру? (y/n/q)')
    user_choice = user_choice.lower()
    while len(user_choice) == 0 or user_choice not in 'ynq':
        print('\n\nНеизвестный ввод\n')
    ...
    if user_choice == 'y':
        ...
        print('Выход из игры. Спасибо, что воспользовались нашей программой')
        break
